...
const (
	...
	module  = "algoexpert-go/algorithm-detail"
)
...
func algoExpert(filePathName string) (string, bytes.Buffer) {
...
func main() {
	...
	title, result := algoExpert(filePathName)
	f := txt.File{
		FilePath:    fmt.Sprintf("%s/%s/", pwd, module),
		FileName:    fmt.Sprintf("%d.%s.md", page, title),
		FileContent: result,
	}
	err := f.Write()
	if err != nil {
